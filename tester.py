...
#                           For evaluation                            #
#######################################################################
def model_test(noise_type, snr, model, validation_loader, direct, dir_to_save, epoch, DEVICE):
    # initialize
    batch_num = 0
    ...
    f_score = open(dir_to_save + '/Epoch_' + '%s_SCORES' % epoch, 'a')
    for inputs, targets in tools.Bar(validation_loader):
        ...
        _, _, outputs = model(inputs, direct)

        if len(outputs[0]) < len(targets[0]):
